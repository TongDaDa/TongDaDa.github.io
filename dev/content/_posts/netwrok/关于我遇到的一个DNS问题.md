...
比如 `.cn` 域名主要是`中国互联网络信息中心（CNNIC）`托管，所以实际上它只需要存储这些`顶级DNS服务机构`的主机地址(IP)就可以。
...
这个阶段，如果你本地有DNS缓存服务的话，它会记住某个域名与它所属的`顶级DNS的IP地址`，下次请求直接去同样类别的顶级DNS服务地址，而不用经过ROOT DNS处理。

`顶级DNS`接到Local DNS请求之后，查询此域名之后发现它已经交给`专属DNS服务器`(也就是你购买域名时设置的域名解析服务器)处理
...
实际上，配置二级，三级等多级的域名都是靠它来的，它更接近与`服务级`, 而上面的顶级DNS属于查找`主域`(企业级)，ROOT DNS 属于`区域级别`(国际域名分区管理)。
...
热情提醒：在使用NODE的 DNS.lookup 或者 resolve 查询时
